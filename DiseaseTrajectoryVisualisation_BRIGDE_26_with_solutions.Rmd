...
merge_cpr_lpr = merge(cpr, lpr, by ="PID")
```
...
merge_cpr_lpr$BIRTHDAY  <- as.Date(merge_cpr_lpr$BIRTHDAY)
merge_cpr_lpr$ADMDATE <- as.Date(merge_cpr_lpr$ADMDATE)

# Type your own commands to calculate the age at each diagnosis 
merge_cpr_lpr$age_at_diagnosis <- as.numeric(
  difftime(merge_cpr_lpr$ADMDATE, merge_cpr_lpr$BIRTHDAY, units = "days")) / 365.25

# calculate the mean age at any diagnosis 
mean(merge_cpr_lpr$age_at_diagnosis)
...
ggplot(merge_cpr_lpr, aes(x = age_at_diagnosis)) +
...
diagnosis_counts <- sort(table(merge_cpr_lpr$CODE), decreasing = TRUE)
head(diagnosis_counts)
```
...
unique_dx <- unique(merge_cpr_lpr[, c("PID", "CODE")])

# Split diagnoses by patient
dx_by_patient <- split(unique_dx$CODE, unique_dx$PID)

# Function to generate diagnosis pairs
get_pairs <- function(dx) {
  if (length(dx) < 2) return(NULL)
  combn(sort(dx), 2, FUN = function(x) paste(x, collapse = " + "))
}

# Generate all diagnosis pairs
all_pairs <- unlist(lapply(dx_by_patient, get_pairs))

# Count comorbidities
comorbidity_counts <- sort(table(all_pairs), decreasing = TRUE)

# Top 10 comorbid diagnosis pairs 
top_10_comorbidities <- head(comorbidity_counts, 10)

print(top_10_comorbidities)
```
